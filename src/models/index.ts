...
//Added this line to control logger 
let isStartUp=true;
setTimeout(() => {
  isStartUp=false;
}, 10000);
...
export const sequelize = new Sequelize(
  ...
  dbConfig.password,
  {
    ...
    pool: {
      max: 15, // Increase available connections
      min: 2,
      acquire: 60000, // Increase timeout (default: 10000ms)
      idle: 10000,
    },
    dialectOptions: {
    ...
    // logging: false,
    logging: (query) => { if (!isStartUp) Logger.info(`DB Query: ${query}`); }
  }
);
