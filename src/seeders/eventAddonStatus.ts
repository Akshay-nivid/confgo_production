...
export = {
  async up(queryInterface: QueryInterface) {
    ...
    const existingStatuses = await queryInterface.sequelize.query(
      `SELECT status_name FROM event_addon_status WHERE status_name IN ('ACTIVE', 'INACTIVE')`,
    ...
    if (statusesToInsert.length > 0) {
      await queryInterface.bulkInsert('event_addon_status', statusesToInsert);
    }
  },

  async down(queryInterface: QueryInterface) {
    await queryInterface.bulkDelete('event_addon_status', {
