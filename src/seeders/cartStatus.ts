...
export = {
  async up(queryInterface: QueryInterface) {
    ...
    const existingStatuses = await queryInterface.sequelize.query(
      `SELECT status_name FROM cart_status WHERE status_name IN ('ACTIVE', 'INACTIVE')`,
    ...
    if (statusesToInsert.length > 0) {
      await queryInterface.bulkInsert('cart_status', statusesToInsert);
    }
  },

  async down(queryInterface: QueryInterface) {
    await queryInterface.bulkDelete('cart_status', {
