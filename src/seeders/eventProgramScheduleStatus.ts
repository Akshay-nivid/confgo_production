...
export = {
  async up(queryInterface: QueryInterface) {
    ...
    const existingStatuses = await queryInterface.sequelize.query(
      `SELECT status_name FROM event_program_schedule_status WHERE status_name IN ('ACTIVE', 'INACTIVE')`,
  ...
  async down(queryInterface: QueryInterface) {
    await queryInterface.bulkDelete('event_program_schedule_status', {
