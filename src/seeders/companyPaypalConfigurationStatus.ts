...
export = {
  async up(queryInterface: QueryInterface) {
    ...
    const existingStatuses = await queryInterface.sequelize.query(
      `SELECT status_name FROM company_paypal_configuration_status WHERE status_name IN ('ACTIVE', 'INACTIVE')`,
    ...
    if (statusesToInsert.length > 0) {
      await queryInterface.bulkInsert('company_paypal_configuration_status', statusesToInsert);
    }
  },

  async down(queryInterface: QueryInterface) {
    await queryInterface.bulkDelete('company_paypal_configuration_status', {
