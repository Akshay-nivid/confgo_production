...
export = {
  async up(queryInterface: QueryInterface) {
    ...
    const existingStatuses = await queryInterface.sequelize.query(
      `SELECT status_name FROM plan_property_assignment_status WHERE status_name IN ('ACTIVE', 'INACTIVE')`,
    ...
    if (statusesToInsert.length > 0) {
      await queryInterface.bulkInsert(
        'plan_property_assignment_status',
        statusesToInsert
      );
    }
  },
  ...
  async down(queryInterface: QueryInterface) {
    await queryInterface.bulkDelete('plan_property_assignment_status', {
