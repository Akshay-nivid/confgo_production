...
interface ICustomCheckbox<T extends FieldValues> {
  ...
  name: Path<T>;
  label?: string;
  options: { label: string; value: string | number }[]; // Array of checkbox options
            ...
            <>
              {options?.map((option) => (
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      checked={Array.isArray(field.value) && field.value.includes(option.value)} // Ensure value is treated as an array
                      onChange={(e) => {
                        const checkedValue = e.target.checked;
                        let newValue: (string | number)[] = [...(field.value || [])]; // Explicitly typing newValue
