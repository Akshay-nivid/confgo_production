...
const LoginOrg = () => {
  ...
  const LoginOrg = async(loginFields:FormData) => {
    try {
      ...
      const { status, data, message } = await processAPIResponse(response, "orgLogin");
      if(status) {
      ...
      setDataById('snackBarInfo', { open: true, autoHideDuration: 2000, severity: 'error', message:error.toString()});
    }
  }
    return (
