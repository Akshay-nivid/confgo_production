...
const SpeakerCard = (eventData: any) => { 
  ...
  const fetchProgramList = async () => {
    try {
      const requestBody = {
        ...
        limit: 5,
        filters: {
          parentEventId: eventData?.eventData?.id,
        },
      };
      setSource({
         method:"POST",
         data:requestBody,
         url: "eventSpeaker/details/list",
  ...
  // };
   ...
   const columns = [
    ...
    { type: "default", field: "name", headerName: "Name", width: 180 },
    {
      type: "default",
      field: "email",
      headerName: "Email",
  ...
  const transformData = (data: any) => {
    ...
    return data.map((item: any) => {
      return {
        ...item,
        id: item?.id,
        profile: <Avatar className='top-2' src={`${baseUrl}/asset/${item?.user?.assetId}`} >{item?.name?.slice(0, 2)}</Avatar>,
