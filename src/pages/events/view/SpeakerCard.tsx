...
const SpeakerCard = (_eventData: any) => {
  ...
  const [editContributorValue, setEditConrtributorValue] = useState<EventParticipant | null>();
      ...
      await POST({
        ...
        successCB: (context: any) => {
          if (context?.success) {
            ...
            setDataById('snackBarInfo', { open: true, autoHideDuration: 2000, severity: 'success', message: "Contributor Added Successfully" });
          }
        },
        errorCB: (context: any) => {
      ...
      await POST({
        ...
        successCB: (context: any) => {
          if (context?.success) {
            ...
            fetchProgramList();
            setAddContributeView(false);
            setDataById('snackBarInfo', { open: true, autoHideDuration: 2000, severity: 'success', message: "Contributor Deleted Successfully" });
          }
        },
        errorCB: (context: any) => {
          setDeleteModal(false);
  ...
  const editContributor = async (formData: FormData) => {
    try {
      ...
      const response = await apiClient.put(`eventProgram/${contributorFields?.id}`, requestBody)
      const { status } = processAPIResponse(response, 'editContribution');
      if (status) {
        fetchProgramList();
        setAddContributeView(false);
        setDataById('snackBarInfo', { open: true, autoHideDuration: 2000, severity: 'success', message: "Contributor Edit Successfully" });
      }
  ...
  return (
    <Grid className="event-detail-speakers-card" container spacing={2} flexDirection={'column'}>
      <Grid container size={{ xs: 12, sm: 12 }}>
          ...
          {contributorList && Object.values(contributorList)?.map((item: any) => {
            return <Grid size={{ xs: 12 }} container flexDirection={"column"} direction={"column"} >
              ...
              <Grid container flexDirection={"row"} direction={"row"}>
                {item.map((item: any) => {
                  return <Grid >
                    <Grid container className="event-detail-speakers-card-list-row-container" key={item.id} alignItems={"flex-start"} spacing={0.5}>
                      <Grid>
                        <img
                        ...
                        <Typography className="event-detail-speakers-card-list-row-type">{item.designation}</Typography>
                        <Grid display={"flex"} className="event-detail-speakers-card-list-row-box">
                          <Grid onClick={() => handleContributorEdit(item)}>
        ...
        {/* Drawer */}
        <CustomDrawer type="right" open={addContributeView} children={
          <Grid className="event-detail-speakers-card-drawer-box">
            <Grid container justifyContent={"space-between"}>
              <Typography className="event-detail-speakers-card-contributor-header">{editContributorValue != null ? "Edit Event Contributor" : "Create New Event Contributor"}</Typography>
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                <Grid container spacing={4}  >
                  ...
                  <Grid container size={{ xs: 12 }} spacing={4}>
                    <Grid size={{ xs: 12 }}>
                      <CustomTextField defaultValue={contributorFields?.name} rules={{ required: true }} control={control} placeholder="Contributor name" name="contributorName" label="Contributor name" requiredField={true} />
                    </Grid>
                    <Grid container size={{ xs: 12 }}>
                    <CustomTextField defaultValue={contributorFields?.name ?? ""} rules={{ required: true }} rows={4} multiline={true} placeholder="Contributor Description" control={control} name="contributorDescription" label="Contributor name" requiredField={true} />
                  </Grid>
                  </Grid>
                ...
                {/* Image Picker */}
                <Grid className="event-detail-speakers-card-btn-container" container justifyContent={"flex-end"} flexDirection={"row"} mr={2}>
                  <CustomButton className="event-detail-speakers-card-btn-container-photo-btn" label="Select Photo" variant="outlined" onClick={() => setModalOpen(true)} />
                  <Grid>
                    {fileRequired && <Typography className="event-detail-speakers-card-btn-container-photo-txt">Please Select an Image</Typography>}
                    {modalOpen && (
                      <FileListModal
                        open={modalOpen}
                  ...
                  <CustomButton className="event-detail-speakers-card-btn-container-submit-btn" label="Submit" variant="contained" type="submit" />
                </Grid>
                {selectedFile && (
                  <Grid
      ...
      >
        <Grid className="event-detail-speakers-card-delete-modal" container justifyContent={"center"}
          alignContent={"center"} flexDirection={"column"} alignItems={"center"} spacing={2} >
          <Typography variant="h6" component="h2">
            {'Do you Want to Delete ?'}
          </Typography>
          <Grid size={{ xs: 6 }} display={"flex"} justifyContent={"space-between"}>
            <CustomButton
