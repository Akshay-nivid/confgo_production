...
const Events = () => {
  ...
  const handleNext = () => {
    if (activeStep < steps.length - 1) {
      const formKey = activeStep === 0 ? "event" : activeStep === 1 ? "program" : activeStep === 2 ? "addOns" : null;
      if (formKey) {
  ...
  const createFormRequest = (data: any, draft?: boolean) => {
    ...
    const transformProgram = program?.map(
      ({
        type,
        addOnId,
        startDate,
    ...
    //tranform addOnData
    const transformedAddOnData = addOn?.map(
      ({
        propertyName,
        propertyAmount,
        repeat,
    ...
    if (event?.type === "ONLINE") {
      req["url"] = event?.url;
      req["meetingUniqueId"] = event?.meetingUniqueId;
    } else {
      req["venue"] = {
  ...
  const onSubmitHandler = (data: object, type: string) => {
    const formKey = type === "EVENT" ? "event" : type === "PROGRAM" ? "program" : type === "ADDS" ? "addOns" : null;
    if (formKey && data) {
  ...
  const transformEventData = (data: any) => {
    ...
    const transformedData = {
      event: {
        ...
        url: data.url || null,
        meetingUniqueId: data.meetingUniqueId || null,
      ...
      program: data.programs?.map((program: any) => ({
        ...
        speakers:
          program?.eventSpeakers?.map((speaker: any) => ({
        ...
        sponsor:
          program?.eventSponsors?.map((sponosr: any) => ({
      ...
      addOns: data.addons?.map((addon: any) => ({
        ...
        noOfDays: "",
        sponsor:
          addon?.eventSponsors?.map((sponosr: any) => ({
            sponsorId: sponosr?.sponsorId,
            sponsorFullName: sponosr?.sponsor?.name,
            speakerLogoId: sponosr?.sponsor?.logoAssetId,
            sponsorTypeId: sponosr?.sponsorTypeId,
                ...
                <Grid>
                  {
                    <CustomButton
                      ...
                      disabled={
                        activeStep === steps.length || (activeStep === 1 && !(formData?.program?.[0]?.name || formData?.program?.[0]?.addonId)) || submitLoader
                      }
                    />
                  }
                </Grid>
