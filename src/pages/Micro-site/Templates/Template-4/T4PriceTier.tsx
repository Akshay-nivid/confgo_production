...
const T4PriceTier = () => {
    return (
        <Box sx={{ backgroundColor: "white" }} >
            <Box className='main t4-tier-section'>
                <h3 className='template-section-title t4-tier-section-title '>Registration & Ticketing</h3>
                                            ...
                                            data && Object?.entries(data)?.map(([key, priceTier], index: number) => {
                                                ...
                                                return (
                                                    <Grid size={{ xs: 10, md: 5 }} key={key} className={clsx('tier-item', isEven ? 'even-item' : 'odd-item')}>
                                                        <h5 className={clsx('tier-item-title', isEven ? "even-tier-title" : "")}>{priceTier[0]?.participantType?.name}</h5>
                                                            ...
                                                            priceTier?.map((item: IEventPriceTier) => (
                                                                <>
                                                                    <Box display={"flex"} justifyContent={"space-between"} className="" key={item?.id}>
                                                                        <Box>
                                                                            <Typography className={clsx('tier-name', isEven ? "even-name" : "")}>{item.name}</Typography>
